...
(function($) { 
	...
	jQuery(document).ready(function(){
		...
		var item_procat = '.product-categories li.cat-parent';
			jQuery(item_procat).append('<span class="icon"></span>'); 
		...
		jQuery('.countdown').each(function(){
			...
			jQuery(this).countdown(countTime, function(event) {
				jQuery(this).html(
					'<div class="timebox day"><div class="timebox-inner">'+'<strong>'+event.strftime('%D')+'</strong>'+'<span>'+junko_countdown_vars.day+'</span></div></div>'+'<div class="timebox hour"><div class="timebox-inner">'+'<strong>'+event.strftime('%H')+'</strong>'+'<span>'+junko_countdown_vars.hour+'</span></div></div>'+'<div class="timebox minute"><div class="timebox-inner">'+'<strong>'+event.strftime('%M')+'</strong>'+'<span>'+junko_countdown_vars.min+'</span></div></div>'+'<div class="timebox second"><div class="timebox-inner">'+'<strong>'+event.strftime('%S')+'</strong>'+'<span>'+junko_countdown_vars.sec+'</span></div></div>'
				);
			});
		});
...
function junkotip(element, content) {
	...
	element.on('mouseover', function(){
		if(jQuery('.junkotip').length == 0) {
			element.before('<span class="junkotip" style="display: none;">'+tipText+'</span>');
