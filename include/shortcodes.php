<?php
function junko_logo_shortcode( $atts ) {
...
function junko_mainmenu_shortcode( $atts ) {
...
function junko_stickymenu_shortcode( $atts ) {
	$junko_opt = get_option( 'junko_opt' );
	$atts = shortcode_atts( array(
		'style'  	=> 'style1',
	), $atts, 'roadstickymenu' );
...
function junko_mobilemenu_shortcode( $atts ) {
	$junko_opt = get_option( 'junko_opt' );
	$atts = shortcode_atts( array(
		'style'  	=> 'style1',
	), $atts, 'roadmobilemenu' );
...
function junko_roadcategoriesmenu_shortcode ( $atts ) {
	$junko_opt = get_option( 'junko_opt' );
	$atts = shortcode_atts( array(
		...
		<div class="categories-menu-wrapper <?php echo esc_attr($atts["style"]) ?>">
			<div class="categories-menu-inner">
				<div class="categories-menu visible-large <?php echo esc_attr($cat_menu_class); ?>">
					<div class="catemenu-toggler"><span><?php if(isset($junko_opt['categories_menu_label'])) { echo esc_html($junko_opt['categories_menu_label']); } else { esc_html_e('ALL CATEGORIES', 'junko'); } ?></span></div>
					<div class="catemenu">
						<div class="catemenu-inner">
							...
							<div class="morelesscate">
								<span class="morecate"><i class="fa fa-plus"></i><?php if ( isset($junko_opt['categories_more_label']) && $junko_opt['categories_more_label']!='' ) { echo esc_html($junko_opt['categories_more_label']); } else { esc_html_e('More Categories', 'junko'); } ?></span>
								<span class="lesscate"><i class="fa fa-minus"></i><?php if ( isset($junko_opt['categories_less_label']) && $junko_opt['categories_less_label']!='' ) { echo esc_html($junko_opt['categories_less_label']); } else { esc_html_e('Close Menu', 'junko'); } ?></span>
							</div>
						</div>
					</div>
				</div>
...
function junko_roadsocialicons_shortcode( $atts ) {
	$junko_opt = get_option( 'junko_opt' );
	$atts = shortcode_atts( array(
		'social_title'      => 'Title',
		'sub_social_title'  => '',
	...
	if(isset($junko_opt['social_icons'])) {
	...
	if(isset($junko_opt['social_icons'])) {
		...
		foreach($junko_opt['social_icons'] as $key=>$value ) {
			if($value!=''){
				if($key=='vimeo'){
...
function junko_roadminicart_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
	), $atts, 'roadminicart' );
	$html = '';
...
function junko_roadwishlist_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
	), $atts, 'roadwishlist' );
	$html = '';
			...
			<div class="header-wishlist <?php echo esc_attr($atts["style"]) ?>">
				<div class="header-wishlist-inner">
					<a href="<?php echo get_permalink(get_option('yith_wcwl_wishlist_page_id')); ?>" class="wishlist-link">
						...
						<span class="wishlist-text"><?php esc_html_e('Wishlist','junko') ?></span>
					</a>
				</div>
			</div>
		<?php } ?>
...
function junko_roadproductssearch_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
  		...
  		<div class="header-search">
  			<div class="search-without-dropdown <?php echo esc_attr($atts["style"]) ?>">
		  		<div class="categories-container">
		  			<div class="cate-toggler-wrapper"><div class="cate-toggler"><span class="cate-text"><?php esc_html_e('All Categories', 'junko');?></span></div></div>
...
function junko_roadproductssearchdropdown_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
...
function junko_brands_shortcode( $atts ) {
	global $junko_opt;
	$brand_index = 0;
	if(isset($junko_opt['brand_logos'])) {
		$brandfound = count($junko_opt['brand_logos']);
	}
	$atts = shortcode_atts( array(
		'rows'             => '1',
		'items_1500up'     => '5',
	...
	if(isset($junko_opt['brand_logos']) && $junko_opt['brand_logos'] && $junko_opt['brand_logos'][0]['url'] != '') {
		$html .= '<div class="brands-carousel roadthemes-slider '.$atts["navigation_style"].' '.$atts["style"].'" data-margin='.$margin.' data-1500up='.$atts["items_1500up"].' data-1200-1499='.$atts["items_1200_1499"].' data-992-1199='.$atts["items_992_1199"].' data-768-991='.$atts["items_768_991"].' data-640-767='.$atts["items_640_767"].' data-480-639='.$atts["items_480_639"].' data-0-479='.$atts["items_0_479"].' data-navigation='.$navigation.' data-pagination='.$pagination.' data-speed='.$atts["speed"].' data-loop='.$loop.' data-auto='.$auto.'>';
			foreach($junko_opt['brand_logos'] as $brand) {
				if(is_ssl()){
...
function junko_imageslider_shortcode( $atts ) {
	global $junko_opt;
	$image_slider_index = 0;
	if(isset($junko_opt['image_slider'])) {
		$image_slider_found = count($junko_opt['image_slider']);
	}
	$atts = shortcode_atts( array(
		'rows'             => '1',
		'items_1500up'     => '4',
	...
	if(isset($junko_opt['image_slider']) && $junko_opt['image_slider'] && $junko_opt['image_slider'][0]['url'] != '') {
		$html .= '<div class="image-slider roadthemes-slider '.$atts["navigation_style"].' '.$atts["style"].'" data-margin='.$margin.' data-1500up='.$atts["items_1500up"].' data-1200-1499='.$atts["items_1200_1499"].' data-992-1199='.$atts["items_992_1199"].' data-768-991='.$atts["items_768_991"].' data-640-767='.$atts["items_640_767"].' data-480-639='.$atts["items_480_639"].' data-0-479='.$atts["items_0_479"].' data-navigation='.$navigation.' data-pagination='.$pagination.' data-speed='.$atts["speed"].' data-loop='.$loop.' data-auto='.$auto.'>';
			foreach($junko_opt['image_slider'] as $image) {
				if(is_ssl()){
...
function junko_counter_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'image'  => '',
		'number' => '100',
		'text'   => 'text',
...
function junko_popular_categories_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'category' => '',
		'image'    => ''
...
function junko_latestposts_shortcode( $atts ) {
	global $junko_opt;
	$post_index = 0;
	$atts = shortcode_atts( array(
		'posts_per_page'   => 10,
	...
	if($atts['image']=='wide'){
		$imagesize = 'junko-post-thumbwide';
	} else {
		$imagesize = 'junko-post-thumb';
	}
	if($atts['category']=='0'){
		$atts['category'] = '';
	}
					...
					$html.='<div class="post-info">';
						$html.='<div class="post-meta">';
							$html.='<p class="post-author">';
								$html.= sprintf( wp_kses(__( '%s', 'junko' ), array('a'=>array('href'=>array()))), esc_html__('By', 'junko').' <a href="'.$author_url.'">'.$author_name.'</a>,' );
							$html.='</p>';
						...
						$html.='<div class="post-excerpt">';
							$html.= Junko_Class::junko_excerpt_by_id($post, $length = $atts['length']);
						$html.='</div>';
					$html.='</div>';
				$html.='</div>';
			$html.='</div>';
...
function junko_testimonials_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'limit'            => '10',
		'orderby'          => 'menu_order',
		'order'            => 'DESC',
...
function junko_magnifier_options($att) {  
	...
	wp_enqueue_script('junko-magnifier', get_template_directory_uri() . '/js/product-magnifier-var.js');
	wp_localize_script('junko-magnifier', 'junko_magnifier_vars', array(
...
function junko_heading_title_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'heading_title'      => 'Title',
		'sub_heading_title'  => '',
		'heading_image'      => '',
	...
	$html.='<div class="heading-title '.$atts["style"].' ">';
		$html.='<div class="heading-title-inner">';
			...
			if ($atts["heading_image"] != '') {
				$html.='<img src="'.wp_get_attachment_url( $atts['heading_image']).'" alt="'.esc_attr__( 'Heading title icon', 'junko' ).'" />';
			}
		$html.='</div>';
	$html.='</div>';
	return $html;
}
function junko_countdown_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'countdown_day'    => '1',
		'countdown_month'  => '1',
		'countdown_year'   => '2020',
...
function junko_newsletter_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'newsletter_title'       => '',
	...
	$html.='<div class="roadthemes-newsletter '.$atts["style"].'">';
		if ($atts["newsletter_title"] != '') {
			...
			if ($atts["newsletter_image"] != '') {
				$html.='<img src="'.wp_get_attachment_url( $atts['newsletter_image']).'" alt="'.esc_attr__( 'Newstter icon', 'junko' ).'" />';
			}
			$html.='</div>';
		}
		ob_start(); ?>
...
function junko_roadthemes_menu( $atts ) {
	$atts = shortcode_atts( array(
		'menu_image'       => '',
		'menu_title'       => 'Title',
		'nav_menu'         => '',
	...
	$html.='<div class="roadthemes-menu '.$atts["style"].'">';
		if ($atts["menu_image"] != '') {
			$html.='<div class="menu-image">';
				$html.='<img src="'.wp_get_attachment_url( $atts['menu_image']).'" alt="'.esc_attr__( 'Menu image', 'junko' ).'"/>';
			$html.='</div>';
		}
		if ($atts["menu_title"] != '') {
...
function junko_roadthemes_policy( $atts ) {
	$atts = shortcode_atts( array(
		'policy_icon'      => '',
		'policy_title'     => 'Title',
		'policy_text'      => '',
		...
		$html.='<div class="policy-inner">';
			if ($atts["policy_icon"] != '') {
				$html.='<div class="policy-icon">';
					$html.='<img src="'.wp_get_attachment_url( $atts['policy_icon']).'" alt="'.esc_attr__( 'Policy icon', 'junko' ).'"/>';
				$html.='</div>';
			}
...
function junko_roadthemes_static_block_1( $atts ) {
	$atts = shortcode_atts( array(
		'static_image'       => '',
		'static_title'       => 'Title',
		'static_text'        => '',
	...
	$html.='<div class="roadthemes-static-1 '.$atts["style"].'">';
		if ($atts["static_image"] != '') {
			$html.='<div class="static-image">';
				$html.='<img src="'.wp_get_attachment_url( $atts['static_image']).'" alt="'.esc_attr__( 'Static image', 'junko' ).'"/>';
			$html.='</div>';
		}
...
function junko_roadthemes_static_block_2( $atts ) {
	$atts = shortcode_atts( array(
		'static_image'       => '',
		'static_title'       => 'Title',
		'static_text'        => '',
