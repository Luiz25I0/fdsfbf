<?php
//WooCommerce Ajax
add_action('wp_head','junko_woo_ajaxurl');
function junko_woo_ajaxurl() {
?>
<?php
...
function product_quickview() {
	...
	<div class="woocommerce product">
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($junko_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<?php if( function_exists('junko_product_sharing') ) { ?>
				<div class="social-sharing"><?php junko_product_sharing(); ?></div>
			<?php } ?>
		</div>
	</div>
	<?php
...
function get_cartinfo() {
	global $woocommerce;
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'junko'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	$product = wc_get_product( $productid );
	?>
	<h3><?php esc_html_e('Product is added to cart', 'junko');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo get_permalink( wc_get_page_id( 'cart' ) );?>"><?php esc_html_e('View Cart', 'junko');?></a>
	</div>
	<?php
	die();
}