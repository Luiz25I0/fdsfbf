...
<head>
<?php $junko_opt = get_option( 'junko_opt' ); ?>
	...
	<div class="page-wrapper">
		<?php 
			if(isset($junko_opt['header_layout']) && $junko_opt['header_layout']!=''){
				$header_class = str_replace(' ', '-', strtolower($junko_opt['header_layout']));
			} else {
				$header_class = '';
			} 
					...
					if ( isset($junko_opt['header_layout']) && $junko_opt['header_layout'] != "") { 
						...
						if(count($jscomposer_templates) > 0) {
							foreach($jscomposer_templates as $jscomposer_template){
								if($jscomposer_template->post_title == $junko_opt['header_layout']){ ?>
						...
						if ( isset($junko_opt['header_mobile_layout']) && $junko_opt['header_mobile_layout'] != "") {
							if(count($jscomposer_templates) > 0) {
								foreach($jscomposer_templates as $jscomposer_template){
									if($jscomposer_template->post_title == $junko_opt['header_mobile_layout']){ ?>
						...
						if ( isset($junko_opt['sticky_header']) && $junko_opt['sticky_header']) {
							if(count($jscomposer_templates) > 0) {
								foreach($jscomposer_templates as $jscomposer_template){
									if($jscomposer_template->post_title == $junko_opt['header_sticky_layout']){ ?>
								...
								<div class="container">
									<div class="row">
										<div class="col-8 col-lg-5 col-xl-4">
											<?php if( isset($junko_opt['logo_main']['url']) && $junko_opt['logo_main']['url']!=''){ ?>
												<div class="logo"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><img src="<?php echo esc_url($junko_opt['logo_main']['url']); ?>" alt=" <?php echo esc_attr( get_bloginfo( 'name', 'display' ) ) ?> " /></a></div>
											<?php } else { ?>
